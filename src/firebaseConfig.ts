  ...
  import { getFirestore, addDoc, collection, getDocs, updateDoc, query, where, getDoc, orderBy, limit } from "firebase/firestore";
  ...
  //update Score1
  const updateScoreInFirebase = async (uid: string, score: number) => {
    try {
      ...
      if (!querySnapshot.empty) {
        ...
        const currentScore = userDoc.data()?.score2 || 0;
  ...
  //Update2 Score
  const updateScore2InFirebase = async (uid: string, score: number) => {
    try {
      // Query to get the user's document in the 'users' collection based on UID
      const userQuery = query(collection(db, 'users'), where('uid', '==', uid));
  ...
  const getTopScores = async (): Promise<{ topScores1: UserScore[], topScores2: UserScore[] }> => {
    const usersCollection = collection(getFirestore(), 'users');
    
    const scoresQuery = query(usersCollection, orderBy('score', 'desc'), limit(10));
    const scores2Query = query(usersCollection, orderBy('score2', 'desc'), limit(10));
